...
#define	BIC_Package	(1 << 0)
#define	BIC_Avg_MHz	(1 << 1)
#define	BIC_Bzy_MHz	(1 << 2)
#define	BIC_TSC_MHz	(1 << 3)
#define	BIC_IRQ		(1 << 4)
#define	BIC_SMI		(1 << 5)
#define	BIC_Busy	(1 << 6)
#define	BIC_CPU_c1	(1 << 7)
#define	BIC_CPU_c3	(1 << 8)
#define	BIC_CPU_c6	(1 << 9)
#define	BIC_CPU_c7	(1 << 10)
#define	BIC_ThreadC	(1 << 11)
#define	BIC_CoreTmp	(1 << 12)
#define	BIC_CoreCnt	(1 << 13)
#define	BIC_PkgTmp	(1 << 14)
#define	BIC_GFX_rc6	(1 << 15)
#define	BIC_GFXMHz	(1 << 16)
#define	BIC_Pkgpc2	(1 << 17)
#define	BIC_Pkgpc3	(1 << 18)
#define	BIC_Pkgpc6	(1 << 19)
#define	BIC_Pkgpc7	(1 << 20)
#define	BIC_PkgWatt	(1 << 21)
#define	BIC_CorWatt	(1 << 22)
#define	BIC_GFXWatt	(1 << 23)
#define	BIC_PkgCnt	(1 << 24)
#define	BIC_Core	(1 << 25)
#define	BIC_CPU		(1 << 26)
#define	BIC_Mod_c6	(1 << 27)

unsigned int bic_enabled = 0xFFFFFFFF;
unsigned int bic_present;

#define DO_CNT(COUNTER_NAME) (bic_enabled & bic_present & COUNTER_NAME)
 ...
 */
unsigned int bic_lookup(char *name_list)
{
	int i;
	...
	while (name_list) {
		...
		for (i = 0; i < MAX_BIC; ++i) {
			if (!strncmp(name_list, bic[i].name, strlen(bic[i].name))) {
				retval |= (1 << i);
				break;
			}
		}
		if (i == MAX_BIC)
			fprintf(stderr, "Ignored invalid counter name: %s\n", name_list);

		name_list = strchr(name_list, ',');
		if (name_list)
			name_list++;
...
void print_header(void)
{
	...
	if (DO_CNT(BIC_Package))
		outp += sprintf(outp, "\tPackage");
	if (DO_CNT(BIC_Core))
		outp += sprintf(outp, "\tCore");
	if (DO_CNT(BIC_CPU))
		outp += sprintf(outp, "\tCPU");
	if (DO_CNT(BIC_Avg_MHz))
		outp += sprintf(outp, "\tAvg_MHz");
	if (DO_CNT(BIC_Busy))
		outp += sprintf(outp, "\tBusy%%");
	if (DO_CNT(BIC_Bzy_MHz))
		outp += sprintf(outp, "\tBzy_MHz");
	if (DO_CNT(BIC_TSC_MHz))
	...
	if (DO_CNT(BIC_CPU_c1))
		outp += sprintf(outp, "\tCPU%%c1");
	if (DO_CNT(BIC_CPU_c3) && !do_slm_cstates && !do_knl_cstates)
		outp += sprintf(outp, "\tCPU%%c3");
	if (DO_CNT(BIC_CPU_c6))
		outp += sprintf(outp, "\tCPU%%c6");
	if (DO_CNT(BIC_CPU_c7))
	...
	if (DO_CNT(BIC_Mod_c6))
		outp += sprintf(outp, "\tMod%%c6");

	if (DO_CNT(BIC_CoreTmp))
	...
	if (DO_CNT(BIC_PkgTmp))
		outp += sprintf(outp, "\tPkgTmp");

	if (DO_CNT(BIC_GFX_rc6))
		outp += sprintf(outp, "\tGFX%%rc6");

	if (DO_CNT(BIC_GFXMHz))
	...
	struct core_data *core_delta)
{
	...
	if (DO_CNT(BIC_Avg_MHz) || DO_CNT(BIC_Busy) || DO_CNT(BIC_Bzy_MHz)) {
...
retry:
	...
	if (DO_CNT(BIC_CPU_c1) && use_c1_residency_msr) {
	...
	if (DO_CNT(BIC_CPU_c6) && !do_knl_cstates) {
...
{
fprintf(stderr, "parse_show_hide: %s: '%s' bic_enabled 0x%x\n", new_mode == HIDE_LIST ? "hide" : "show", optarg, bic_enabled);
